...
pub fn main() -> i32 {
    ...
    pipe(&mut pipe_fd);
    // read end
    println!("pipe_fd[0](read end): {}", pipe_fd[0]);
    assert_eq!(pipe_fd[0], 3);
    // write end
    println!("pipe_fd[1](write end): {}", pipe_fd[1]);
    ...
    if fork() == 0 { 
        ...
        println!("Read from parant: {:?}", core::str::from_utf8(&buffer[..len_read]).unwrap());
        0
    } else {
        // parent process, write to child
        // close read end
        close(pipe_fd[0] as usize);
        println!("Write to child: {:?}", STR);
        assert_eq!(
