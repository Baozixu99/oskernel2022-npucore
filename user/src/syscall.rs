...
pub fn sys_dup(fd: usize) -> isize {
    syscall(SYSCALL_DUP, [fd, 0, 0,0])
}

pub fn sys_open(path: &str, flags: u32) -> isize {
    syscall(SYSCALL_OPEN, [path.as_ptr() as usize, flags as usize, 0,0])
}
...
pub fn sys_pipe(pipe: &mut [i32]) -> isize {
    syscall(SYSCALL_PIPE, [pipe.as_mut_ptr() as usize, 0, 0,0])
}
...
pub fn sys_get_time() -> isize {
    syscall(SYSCALL_GET_TIME, [0, 0, 0,0])
}
...
pub fn sys_fork() -> isize {
    ...
    syscall(SYSCALL_CLONE, [SIGCHLD, 0, 0,0])
}

pub fn sys_exec(path: &str, args: &[*const u8], envp: &[*const u8]) -> isize {
    syscall(
        SYSCALL_EXECVE,
        [path.as_ptr() as usize, args.as_ptr() as usize, envp.as_ptr() as usize,0],
    )
}

pub fn sys_waitpid(pid: isize, exit_code: *mut i32) -> isize {
    syscall(SYSCALL_WAIT4, [pid as usize, exit_code as usize, 0,0])
}

pub fn sys_mycall() -> isize {
    syscall(SYSCALL_MYCALL,[0,0,0,0])
}

pub fn sys_printtcb(message: *mut usize) -> isize {
    syscall(SYSCALL_PRINT_TCB, [message as usize, 0, 0,0])
}

pub fn sys_getrusage(who: isize, rusage:*mut usize) -> isize {
    syscall(SYSCALL_GETRUSAGE, [who as usize, rusage as usize, 0,0])
}
pub fn sys_fstat(fd: usize,stat:*mut Stat) -> isize{
    syscall(SYSCALL_FSTAT, [fd,stat as usize,0,0])
}
pub fn sys_fstatat(dirfd: usize,path:&str,stat:*mut Stat,flags: u32) -> isize{
    syscall(SYSCALL_NEW_FSTATAT, [dirfd,path.as_ptr() as usize,stat as usize,flags as usize])
}
pub fn sys_linkat(olddirfd: usize, oldpath: &str, newdirfd: usize, newpath: &str) -> isize {
    syscall(
        SYSCALL_LINKAT,
        [
            olddirfd,
...
pub fn sys_unlinkat(dirfd: usize, path: &str, flags: u32) -> isize {
    syscall(SYSCALL_UNLINKAT, [dirfd, path.as_ptr() as usize, flags as usize,0])
}
pub fn sys_getdents64(fd: usize, buf: &mut [u8]) -> isize {
    syscall(SYSCALL_GETDENTS64, [fd, buf.as_mut_ptr() as usize, buf.len(),0])
}