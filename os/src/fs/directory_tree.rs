...
pub struct DirectoryTreeNode {
    //如果这是一个目录，表示此节点的特殊用途的数量，如当前工作目录、挂载点或根节点
    //如果这是一个文件，表示正在被一些进程执行的数量。每次打开文件时，该参数加 1
    spe_usage: Mutex<usize>,    
    name: String,                   //节点的名称     
    filesystem: Arc<FileSystem>,    //指向文件系统的智能指针
    file: Arc<dyn File>,            //提供对文件操作的抽象
    selfptr: Mutex<Weak<Self>>,     //指向自己的指针，主要用于引用计数管理和避免循环引用
    father: Mutex<Weak<Self>>,      //指向父目录节点，用于建立目录树的层次结构
    //是子节点的名称，值是指向子节点的智能指针。如果这是一个目录节点，这个字段将包含该目录的子节点
...
impl DirectoryTreeNode {
    ...
    pub fn open(
        ...
        info!("[open]: cwd: {}, path: {}", self.get_cwd(), path);
