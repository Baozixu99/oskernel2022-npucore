...
pub struct PipeRingBuffer {
    // 固定大小的字节数组，用于存储缓冲区的数据
    arr: Box<[u8; RING_DEFAULT_BUFFER_SIZE]>,
    // 缓冲区的头部索引，指向下一个要读取的位置
    head: usize,
    // 缓冲区的尾部索引，指向下一个要写入的位置
    tail: usize,
    // 缓冲区的状态，可能表示缓冲区是空的、满的或其他状态
    status: RingBufferStatus,
    // 一个可选的弱引用，用于跟踪哪个Pipe对象正在向缓冲区写入数据。使用 Weak 引用是为了避免循环引用导致的内存泄漏
    write_end: Option<Weak<Pipe>>,
    // 一个可选的弱引用，用于跟踪哪个 Pipe 对象正在从缓冲区读取数据。使用 Weak 引用是为了避免循环引用导致的内存泄漏
...
impl PipeRingBuffer {
    ...
    fn buffer_read(&mut self, buf: &mut [u8]) -> usize {
        ...
        let end = if self.tail <= self.head {
            RING_DEFAULT_BUFFER_SIZE    //缓冲区的末尾
        } else {
            self.tail
        };
        // copy 取目标缓冲区 buf 的长度和 end - begin 的最小值，确保不会读取超过可用数据或目标缓冲区的大小。
    ...
    fn all_write_ends_closed(&self) -> bool { //如果 upgrade() 返回 None，表示所有写入端都已经关闭。
...
impl File for Pipe {
    ...
    fn read_user(&self, offset: Option<usize>, buf: UserBuffer) -> usize {
        ...
        loop {
            ...
            let mut ring = self.buffer.lock();
            if ring.status == RingBufferStatus::EMPTY {
                if ring.all_write_ends_closed() { //如果缓冲区为空且所有写入端都已关闭，则返回已读取的字节数。
                    return read_size;
                }
                drop(ring); 
                suspend_current_and_run_next(); //如果缓冲区为空但写入端未关闭，则挂起当前任务并继续尝试读取。
                continue;   
            }
            // We guarantee that this operation will read at least one byte
            // So we modify status first
            for buf in buf.buffers {  //遍历用户提供的每个缓冲区片段。
                let mut buf_start = 0;   //用于记录当前缓冲区片段中已读取的字节数。
                //在当前缓冲区片段中循环读取数据，直到读取满整个缓冲区片段。
                while buf_start < buf.len() {
                    let read_bytes = ring.buffer_read(&mut buf[buf_start..]); //从环形缓冲区读取数据到当前缓冲区片段
                    buf_start += read_bytes;
                    if ring.head == ring.tail {  //如果环形缓冲区的头尾指针相等，表示h环形缓冲区已空。
                        ring.status = RingBufferStatus::EMPTY;
                        read_size += buf_start;
                        return read_size;
                    }
