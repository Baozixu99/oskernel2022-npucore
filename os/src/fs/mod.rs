...
pub struct FileDescriptor {
    cloexec: bool,   //是否设置了 close-on-exec 标志
    nonblock: bool,  //是否设置为非阻塞模式
...
impl FileDescriptor {
    ...
    pub fn open(&self, path: &str, flags: OpenFlags, special_use: bool) -> Result<Self, isize> {
        ...
        //可改写成下面
        // let inode =  match self.file.get_dirtree_node() {
        //     Some(inode) => inode,
        //     None => return Err(ENOENT),
        // };
...
impl FdTable {
    ...
    pub fn insert(&mut self, file_descriptor: FileDescriptor) -> Result<usize, isize> {
        let fd = match self.recycled.pop() {   //将recycle中一个文件描述符pop出来进行判断是不是需要插入的新的文件描述符
            Some(fd) => {                          //这里很奇怪为什么只pop出一个来判断，而不是查找recycle中的所有fd
                self.inner[fd as usize] = Some(file_descriptor);
                fd as usize
            },
            None => {
                let current = self.inner.len(); //current为文件描述符表数组的下标
