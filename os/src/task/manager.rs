...
impl TaskManager {
    ...
    pub fn find_by_pid(&self, pid: usize) -> Option<Arc<TaskControlBlock>> {
        self.ready_queue
            .iter()
            .chain(self.interruptible_queue.iter()) //将ready_queue和interruptible_queue连接起来
...
impl WaitQueue {
    ...
    pub fn add_task(&mut self, task: Weak<TaskControlBlock>) {  //弱引用
...
impl TimeoutWaitQueue {
    ...
    pub fn wake_expired(&mut self, now: TimeSpec) {
        ...
        while let Some(waiter) = self.inner.pop() {
            ...
            //从超时等待池中取出的进程的等待时间进⾏⽐较，若还未超时，则将该进程压回⼆叉堆。
            ...
            // wake one task
                ...
                match waiter.task.upgrade() {  
                    Some(task) => {  //upgrade() 成功返回 Some(task)，则表示任务仍然存在。
                        let mut inner = task.acquire_inner_lock();
                        match inner.task_status {
                            super::TaskStatus::Interruptible => {
                                inner.task_status = super::task::TaskStatus::Ready
                        ...
                        manager.wake_interruptible(task);  //唤醒task
                    }
                    // task is dead, just ignore
                    None => continue,
                }
