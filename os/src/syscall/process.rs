...
bitflags! {
    pub struct CloneFlags: u32 {
        ...
        const CLONE_VM              =   0x00000100; //⽗⼦进程会共享同⼀个虚拟内存⻚，共享同⼀虚拟内存是线程的关键属性之⼀
        const CLONE_FS              =   0x00000200; //⼦进程使⽤⽗进程的⽂件系统状态
        const CLONE_FILES           =   0x00000400; //⽗、⼦进程会共享同⼀个打开⽂件描述符表
        const CLONE_SIGHAND         =   0x00000800; //⽗⼦进程将共享同⼀个信号处置表
        const CLONE_PIDFD           =   0x00001000;
        const CLONE_PTRACE          =   0x00002000; //允许子进程通过ptrace系统调用来追踪父进程的执行情况。
        const CLONE_VFORK           =   0x00004000;
        const CLONE_PARENT          =   0x00008000; //那么创建的⼦进程的⽗进程是调⽤者的⽗进程，新进程与创建它的进程成了“兄弟”⽽不是“⽗⼦”。
        const CLONE_THREAD          =   0x00010000; //会将⼦进程置于⽗进程的线程组中。如果未设置，那么会将⼦进程置于新的线程组中。
        const CLONE_NEWNS           =   0x00020000;
        const CLONE_SYSVSEM         =   0x00040000; //则⼦进程使⽤⽗进程的System V信号量
        const CLONE_SETTLS          =   0x00080000; //产⽣的线程⽀持TLS
        const CLONE_PARENT_SETTID   =   0x00100000; //表示子线程的线程标识符将被存储在父进程内存中由 ptid 指向的位置
        const CLONE_CHILD_CLEARTID  =   0x00200000; //则会在⼦进程终⽌时将ctid所指向的内存清零
        const CLONE_DETACHED        =   0x00400000;
        const CLONE_UNTRACED        =   0x00800000;
        const CLONE_CHILD_SETTID    =   0x01000000; //表示子线程的线程标识符将被存储在子进程内存中由 ctid 指向的位置
...
pub fn sys_clone(   //封装后的sys_clone
    ...
    if flags.contains(CloneFlags::CLONE_CHILD_SETTID) {     //表示子线程的线程标识符将被存储在子进程内存中由 ctid 指向的位置
        match translated_refmut(child.get_user_token(), ctid) {//这使得子进程内部也能立即知道自己的ID
            Ok(word) => *word = child.pid.0 as u32,
            Err(errno) => return errno,
        };
    }
...
pub fn sys_execve(
    ...
    mut argv: *const *const u8, //新程序的命令行参数表，指向⽤户空间的参数列表起始地址
...
bitflags! {
    struct WaitOption: u32 {
        const WNOHANG    = 1;  //获取导致子进程停止的信号编号
        const WSTOPPED   = 2;
        const WEXITED    = 4;
        const WCONTINUED = 8;
...
pub fn sys_printtcb(message: *mut usize) -> isize {
    ...
    let token = tcb.get_user_token();  //获取物理页标识符
