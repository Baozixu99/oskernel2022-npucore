...
pub fn sys_fstatat(dirfd: usize, path: *const u8, buf: *mut u8, flags: u32) -> isize {
    ...
    info!("[sys_fstatat] dirfd: {}, path: {:?}, flags: {:?}",dirfd as isize, path, flags,);
    let task = current_task().unwrap();
    let file_descriptor = match dirfd {         
        AT_FDCWD => task.fs.lock().working_inode.as_ref().clone(),
        fd => {
    ...
    match file_descriptor.open(&path, OpenFlags::O_RDONLY, false) {  //获取打开文件的新的文件描述符
        Ok(file_descriptor) => {//调用copy_to_user将文件信息内容拷贝到buf中
            // let fds= &file_descriptor.get_stat();
            // print!("fds: {:?}",fds);
...
pub fn sys_fstat(fd: usize, statbuf: *mut u8) -> isize {
    ...
    let file_descriptor = match fd {    //获取当前进程的文件描述符 
        AT_FDCWD => task.fs.lock().working_inode.as_ref().clone(),
        fd => {
            let fd_table = task.files.lock();
            match fd_table.get_ref(fd) {
   ...
   // let fds= &file_descriptor.get_stat();
    //print!("fds: {:?}",fds);
    //调用copy_to_user将文件信息内容拷贝到statbuf中
...
pub fn sys_openat_(dirfd: usize, path: *const u8, flags: u32, mode: u32) -> isize {  
    let task = current_task().unwrap();
    let token = task.get_user_token();
    let mut fd_table = task.files.lock();
