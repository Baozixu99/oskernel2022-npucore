...
pub struct MemorySet {
    ...
    areas: Vec<MapArea>,  //表示已映射的区域,每个元素都是一个 MapArea 结构体，用于描述不同段的内存映射情况.
}
...
impl MemorySet {
    ...
    pub fn token(&self) -> usize {  //物理页的标识符（token）
    ...
    //enumerate()：将迭代器转换为一个元素及其索引的元组序列。
    //rev()：将元素的迭代顺序反转，从最后一个元素开始向前迭代。
    //skip(2)：跳过前两个元素，从第三个元素开始迭代。
    ...
    pub fn from_existing_user(user_space: &mut MemorySet) -> MemorySet {
        ...
        for i in 0..user_space.areas.len() - 1 {  //该循环方式，不会达到右端点，从而实现不遍历最后一个区域，因为最后一个区域是trap,会单独处理
        ...
        // copy trap context area   //处理最后一个区域
    ...
    pub fn mmap(
        ...
        let start_va: VirtAddr = if flags.contains(MapFlags::MAP_FIXED) {  //MAP_FIXED使用指定的映射起始地址
            ...
            if let Some(idx) = idx {
                ...
                if flags.contains(MapFlags::MAP_PRIVATE | MapFlags::MAP_ANONYMOUS)
                    && prot == area.map_perm   // 增加条件，判断区域权限与参数中指定权限相同
                    && area.map_file.is_none() // 增加条件，判断区域 map_file 为空
                {
                    ...
                    // let old_end:VirtAddr = area.inner.vpn_range.get_end().into();
                    // let new_end:VirtAddr = (old_end.0+len).into();
                    // area.expand_to(new_end).unwrap();
        ...
        let (idx, _) = self
            ...
            .iter()
            .enumerate()
            .skip_while(|(_, area)| {area.inner.vpn_range.get_start() >= VirtAddr::from(MMAP_END).into()})
...
#[derive(Clone, Debug)]
pub enum Frame {
    InMemory(Arc<FrameTracker>),// 表示该页帧在内存中，并且被一个 FrameTracker 所管理
    Unallocated,                // 表示该页帧尚未被分配
}
...
#[derive(Clone)]
pub struct LinearMap {
    vpn_range: VPNRange,    //映射的虚拟页表号范围
    frames: Vec<Frame>,     //Frame用来记录该物理页帧是否被分配
}
...
pub struct MapArea {
    inner: LinearMap,  //用于跟踪物理页框到虚拟页的映射关系。
    map_type: MapType,
...
impl MapArea {
    ...
    pub fn map_from_existing_page_table(
        ...
        let map_perm = self.map_perm.difference(MapPermission::W);  //除去写权限
    ...
    /// 从给定的 PageTable（页面表）中取消映射当前内存区域所包含的所有页面。此方法遵循了懒惰性分配策略，允许部分页面未被映射。
    pub fn unmap(&mut self, page_table: &mut PageTable) -> Result<(), MemoryError> {
        let mut has_unmapped_page = false;  //用于追踪是否至少有一个页面在取消映射过程中被发现未映射。
        for vpn in self.inner.vpn_range {
    ...
    pub fn shrink_to(&mut self, page_table: &mut PageTable, new_end: VirtAddr) -> Result<(), ()> {
        ...
        for vpn in VPNRange::new(new_end_vpn, old_end_vpn) {  
            if let Err(_) = self.unmap_one(page_table, vpn) {   //取消new_end_vpn到old_end_vpn之间的映射
                has_unmapped_page = true;
            }
        }
    ...
    pub fn rshrink_to(
        ...
        let mut has_unmapped_page = false;
        for vpn in VPNRange::new(old_start_vpn, new_start_vpn) {  //取消old_start_vpn到new_start_vpn之间的映射区域
    ...
    pub fn check_overlapping(
        ...
        if end_vpn < area_start_vpn || start_vpn >= area_end_vpn {
            ...
            let start = if start_vpn > area_start_vpn {  //start 是两个区域起始页中较大的那个，确保落在两者交集的起始点。
                start_vpn
            } else {
                area_start_vpn
            };
            let end = if end_vpn < area_end_vpn {  //end 是两个区域结束页中较小的那个，确保落在交集的终点。
                end_vpn
            } else {
                area_end_vpn
            };
            return Some((start, end));    //返回重叠区域
        }
    }
    pub fn into_two(self, cut: VirtPageNum) -> Result<(Self, Self), ()> {
