...
impl PageTable {
    ...
    pub fn print_page_table(&self){
        let mut ppn = self.root_ppn;  //root_ppn是一个页表的唯一标识
        ...
        for pte_zero in ppn.get_pte_array(){   //遍历一级页表项数组,get_pte_array可获取页表项数组
            ...
            number_zero += 1;
            for pte_one in ppn.get_pte_array(){   //遍历二级页表项数组
                if !pte_one.is_valid(){
                    number_one += 1;
                   continue;
                }
                ...
                number_one += 1;
                for pte_two in ppn.get_pte_array(){//遍历三级页表项数组
                    if !pte_two.is_valid(){
                        number_two += 1;
                        continue;
                    }
    ...
    fn find_pte_refmut(&self, vpn: VirtPageNum) -> Option<&mut PageTableEntry> {
        ...
        for i in 0..3 {
            ...
            // if i == 0 {
            //     println!("..{:x}: pte 0x{:x} pa 0x{:x}",i,pte.bits,pa.0);
            // }
            // if i == 1 {
            //     println!(".. ..{:x}: pte 0x{:x} pa 0x{:x}",i,pte.bits,pa.0);
            // }
...
/// 这个函数的作用是将以 ptr 指向的以 null 结尾的字符串转换为 Rust 的 String 类型。它从指定的虚拟内存地址开始，不断地读取内存中的字节数据，直到遇到空字符（null 终止符）为止，并将这些字节数据逐个添加到 String 中。最后返回构建好的字符串。
/// 跨地址空间加载字符串数据，它会在处理每个字节时进行地址转换，并将读取的字节构造为一个字符串，直到遇到字符串的结束符号 \0。
pub fn translated_str(token: usize, ptr: *const u8) -> Result<String, isize> {
    ...
    loop {
        let ch: u8 = *({
            ...
            let pa = match page_table.translate_va(va) {  //查找给定虚拟地址在页表中的映射，如果映射存在，则返回对应的物理地址pa
                Some(pa) => pa,
                None => check_page_fault(va)?,
            };
            pa.get_mut()  //获取物理地址处的字节数据
        });
        if ch == 0 {
            break;
        }
        string.push(ch as char);
        cur += 1;  //从虚拟地址cur开始，不断地读取地址中的字节数据
    }
    Ok(string)
}

/// Translate the user space pointer `ptr` into a reference in user space through page table `token`
/// 此函数内核读取指针所指向的用户空间数据，并返回对应数据的内核地址引用。它执行地址转换并尝试将用户空间地址转换为内核空间地址。
...
/// 将用户空间中的一个指针（ptr）转换为内核空间可变引用,并通过给定的页表（通过token标识）进行地址转换，以便内核空间可以安全地访问和修改用户空间的数据
